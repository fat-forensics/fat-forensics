...
import fatf.transparency.models.feature_importance as ftmfi
...
categorical_steps_warning = ('Feature is defined as categorical but number of '
                             ...
                             'for feature.')
is_categorical_false_warning = ('Feature %s is not numerical and not '
                                'specified as categorical. Samples will be '
                                'generated by using values contained in the '
                                'dataset.')
...
def test_individual_conditional_expectation():
    ...
    ice_struct_cat = ftmfi.individual_conditional_expectation(
        test_structure_array, predictor_struct, 'a', is_categorical=True)
    assert np.allclose(ice_struct_cat[0], results_cat)
    assert np.allclose(ice_struct_cat[1], values_cat_struct)

    predictor_string = fum.KNN(k=2)
    predictor_string.fit(string_structure_array, np.array([0, 1, 1]))
    with pytest.raises(UserWarning) as exin:
        ftmfi.individual_conditional_expectation(string_test_structure_array,
                                                 predictor_string, 'feat3')
...
def test_partial_dependence():
    ...
    predictor.fit(numerical_array, np.array([2, 0, 1, 1, 0, 2]))
    with pytest.raises(IncorrectShapeError) as exin:
        ftmfi.partial_dependence(np.ones((6, 4, 4)), predictor, 3, steps=3)
    assert str(exin.value) == incorrect_shape_msg

    invalid_model = InvalidModel()
    with pytest.raises(IncompatibleModelError) as exin:
        ftmfi.partial_dependence(
            test_numerical_array, invalid_model, 3, steps=3)
    assert str(exin.value) == model_err_msg

    with pytest.raises(ValueError) as exin:
        ftmfi.partial_dependence(test_numerical_array, predictor, 10, steps=3)
    assert str(exin.value) == indices_error_msg

    pd = ftmfi.partial_dependence(test_numerical_array, predictor, 3, steps=3)
    assert np.allclose(pd[0], np.mean(results, axis=0))
    assert np.allclose(pd[1], values)

    with pytest.raises(UserWarning) as exin:
        pd_cat = ftmfi.partial_dependence(
            test_numerical_array, predictor, 0, is_categorical=True, steps=3)
    assert str(exin.value) == categorical_steps_warning
    pd_cat = ftmfi.partial_dependence(
        test_numerical_array, predictor, 0, is_categorical=True)
