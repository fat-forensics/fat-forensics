...
import fatf.utils.arrays.tools as fuat
...
class Mixup(object):
    ...
    def __init__(self,
        ...
        self.numerical_indices, self.categorical_indices = fuat.indices_by_type(dataset)
    ...
    def sample(self,
        ...
        if not subject_instance.dtype == self.dataset.dtype:
            raise ValueError('The input instance should have the same dtype as the input dataset')
        ...
        if not self.balanced:
            ...
            for clss, count in zip(unique, counts):
                ...
                n_tosample = int((count / total) * n_draws)
        ...
        random_draws_lambda = np.random.beta(self._beta_parameters[0],
                                             self._beta_parameters[1],
                                             n_draws)

        if self.structure_bool:
            sampled_x = np.empty((n_draws, ), dtype=self.dataset.dtype)
        else:
            sampled_x = np.zeros((n_draws, self.dataset.shape[1]), dtype=self.dataset.dtype)

        if len(self.y.shape) == 1:
            sampled_y = np.zeros(n_draws).reshape(-1, 1)
        else:
            n_classes = self.y.shape[1]
            sampled_y = np.zeros((n_draws, n_classes)).reshape(-1, n_classes)
        ...
        for i in range(n_draws):
            ...
            for ftr in self.numerical_indices:
                sampled_x[i][ftr] = (1 - l) * random_draw_x[ftr] \
                                    + l * subject_instance[ftr]
            for ftr in self.categorical_indices:
                if l <= self.threshold:
                    sampled_x[i][ftr] = subject_instance[ftr]
                else:
